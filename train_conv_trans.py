...
def train(data, img_flag):
	...
	for e in range(args.epochs):
		...
		for idx in range(batch_num):
			...
			img_ir, ori_imgs_ir = utils.get_train_images(image_paths_ir, height=args.Height, width=args.Width, flag=img_flag)

			image_paths_vi = [x.replace('lwir', 'visible') for x in image_paths_ir]
			img_vi, ori_imgs_vi = utils.get_train_images(image_paths_vi, height=args.Height, width=args.Width, flag=img_flag)

			ori_imgs = (ori_imgs_ir, ori_imgs_vi)
			count += 1
			optimizer.zero_grad()
			...
			# for DataParallel
			outputs = model.train_module(batch_ir, batch_vi, ir_sh, vi_sh, ir_de, vi_de, shift_flag, gra_loss, order_loss, ori_imgs, idx, e)
